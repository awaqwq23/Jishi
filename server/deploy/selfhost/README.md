...
此方案由四个独立低内存服务组成：Web 客户端、API 服务器、本地 SQLite/D1 运行时和 GitHub OAuth 网关。总内存上限约 704MB，业务数据和图片持久化在 `jishi_data` 卷。

1. 在 GitHub 创建 OAuth App，回调地址填写 `https://你的域名/oauth2/callback`。
2. 复制 `server/.env.selfhost.example` 为仓库根目录的 `.env` 并填写三项 OAuth 值。
