...
feature_commit=b410f251e7ee5da5d7223fb83089bdf836e30108
stage=/opt/jishi-stage-b410f25
backup=/var/backups/jishi/pre-b410f25-20260918-github-update
old_backups=(
  /var/backups/jishi/pre-02a1aea-20260916-progress-notifications
)
...
test -f "$stage/server/deploy/native/releases/deploy-b410f25-20260918.sh"
test "$(realpath "$stage/server/deploy/native/releases/deploy-b410f25-20260918.sh")" = "$stage/server/deploy/native/releases/deploy-b410f25-20260918.sh"
...
test -f "$downloads/Jishi-Windows-Setup-0.4.6.exe"
test -f "$downloads/Jishi-Android-0.4.7.apk"
sha256sum -c <<'HASHES'
4079bca14b51a731d7bbc230dfeaaaaa6ffc7ed184403f10880167a8a5c92244  /opt/jishi-stage-b410f25/Jishi-Windows-Setup-0.4.6.exe
2c238d4f4d346f613d77e008a626ef46a98f5f70fec6f3e84f089fff2c0fa8e2  /opt/jishi-stage-b410f25/Jishi-Android-0.4.7.apk
e0d3993ed51c13f8f6fcfbbe63d42c1e8913e1e72e9a094dc5f4befb8dfa223d  /opt/jishi-stage-b410f25/clients/web/public/updates/latest.json
...
(cd "$stage" && node scripts/publish-github-release.mjs --verify)
...
tar -cpf "$backup/production.tar" -C / \
  ...
  etc/nginx/sites-available/jishi etc/nginx/sites-available/jishi-subdomain \
...
assert expected['releaseId'] == 'web-1eb33d5ff18eb60d'
assert expected['webBuild'] == '1eb33d5ff18eb60d1658b947aff42c57f71231e47bedef1946bd6eb95319acfc'
...
e0d3993ed51c13f8f6fcfbbe63d42c1e8913e1e72e9a094dc5f4befb8dfa223d  /var/www/jishi-downloads/latest.json
6b78ae9f26c6adb819ff0be45c5a72ee300002baec866baabf59ae8d7e591781  /etc/nginx/sites-available/jishi
LIVE_HASHES
systemctl is-active --quiet jishi-api
systemctl is-active --quiet jishi-web
...
trap - ERR
# Keep both backups and the stage until a real account signs in and reads
# bootstrap on a device. The automated signed-session probe is not that check.
...
test "$(realpath "$stage")" = /opt/jishi-stage-b410f25
echo "deployment=automated-pass feature_commit=$feature_commit backup=$backup real_account_check=pending cleanup=pending"
