...
export default function TodoApp({ authenticatedUser }: { authenticatedUser: ChatGPTUser | null }) {
  ...
  const uploadImage = async (file: File, kind: "avatar" | "background") => {
    const response = await fetch(`/api/media?kind=${kind}`, { method: "PUT", headers: { "Content-Type": file.type }, body: file });
  ...
  if (!authenticatedUser && error.includes("登录")) return <main className="signin-screen"><div className="brand-mark">记</div><span className="eyebrow">欢迎来到记时</span><h1>把今天，安放得刚刚好。</h1><p>登录后，你的待办、提醒与个人设置会在四端保持一致。</p><a className="button primary" href="/signin-with-chatgpt?return_to=%2F"><LogIn size={18} />使用 ChatGPT OAuth 登录</a></main>;
  ...
  return <div className={`app-shell acrylic-${settings.acrylic}`} style={{ "--accent": settings.accent, "--app-background-image": settings.backgroundUrl ? `url(${settings.backgroundUrl})` : "none" } as React.CSSProperties}>
    <aside className="sidebar">
      ...
      <div className="sidebar-profile"><span className="avatar">{data.user.avatarUrl ? <Image src={data.user.avatarUrl} alt="用户头像" fill sizes="38px" unoptimized /> : data.user.name.slice(0, 1)}</span><div><strong>{data.user.name}</strong><small>已同步</small></div><ChevronRight size={17} /></div>
    </aside>
